    ...
    return (
      ...
      >
        <AccordionPrimitive.Trigger
          className={cn(
            `flex items-center gap-1 rounded-md text-base`,
            className,
            {
        ...
        >
          {expandedItems?.includes(value)
            ? (openIcon ?? <FolderOpenIcon className="size-6" />)
            : (closeIcon ?? <FolderIcon className="size-6" />)}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
        <AccordionPrimitive.Content className="relative h-full overflow-hidden text-base data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
          ...
          <AccordionPrimitive.Root
            dir={direction}
            type="multiple"
            className="relative ml-5 flex flex-col gap-1 py-1 rtl:mr-5"
            defaultValue={expandedItems}
            value={expandedItems}
            onValueChange={(value) => {
              setExpandedItems?.((prev) => [...(prev ?? []), value[0]]);
    ...
    return (
      <AccordionPrimitive.Item value={value} className="relative">
        <AccordionPrimitive.Trigger
          ...
          aria-label="File"
          className={cn(
            "flex cursor-pointer items-center gap-1 rounded-md pr-1 text-base duration-200 ease-in-out rtl:pl-1 rtl:pr-0",
            {
        ...
        >
          {fileIcon ?? <FileIcon className="size-6" />}
          {children}
        </AccordionPrimitive.Trigger>
      </AccordionPrimitive.Item>
    );
